...
A: 不可以。雖然相同類別內的敘述可以直接存取案例變數、靜態變數或常數，無須建立類別的物件或指定類別的名稱，但也有例外，
   就是靜態方法內的敘述 可以直接存取靜態變數或常數，但不能直接存取案例變數，必須建立類別的物件，理由很簡單，
   若使用者是透過類別的名稱呼叫靜態方法，沒有建立類別的物件，那麼將因為無法存取案例變數而產生編譯錯誤。
...
抽象方法、
密封方法等
由於這些方法和類別的繼承有關，所以此處暫不討論，

=> 在使用 static 關鍵字宣告靜態 方法時，不能再加上 virtual 或 abstract 關鍵字將它宣告為虛擬方法或抽象 方法。
=> 在使用 static 關鍵字宣告靜態 屬性時，不能再加上 virtual 或 abstract 關鍵字將它宣告為虛擬方法或抽象 屬性。
